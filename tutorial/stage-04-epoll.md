...
> **S3 的线程池代码去哪了?** 从本阶段到 S8,main.cpp 都是单线程事件驱动,S3 的 `threadpool.h` 暂时"退场"——但它没有白学:epoll 解决"**等事件**",线程池解决"**处理并发**",两者是互补关系。S9 整合时会把**原版**线程池(带 actor 模型和连接池成员)装回来,与事件循环合体,那才是原项目的最终形态。`lock/locker.h` 则更早一步,在 S7 以原版回归(异步日志的阻塞队列需要 `cond::timewait`)。
...
原项目用参数 `-a` 切换:**`-a 0` 是模拟 Proactor(默认,主线程读完再交给工作线程),`-a 1` 是 Reactor(工作线程自己读)**。Stage 9 整合时会看到两种代码路径。
...
| 4 | **大包测试**:`head -c 5000 /dev/zero \| nc 127.0.0.1 9006 \| wc -c` | 输出 `5000`(证明 ET 循环读到 EAGAIN 正确) | ☐ |
| 5 | **慢客户端测试**:一个 nc 连上后不发任何字,再开另一个 nc | 第二个正常回显(单线程也扛得住,因为不阻塞) | ☐ |
| 6 | 服务器日志 | 显示各连接的新建与关闭 | ☐ |

> **第 4 条最检验 ET 写没写对**:`/dev/zero` 一次性吐出 5000 个字节(超过读缓冲区 1024),如果 ET 模式只 `read` 一次就退出循环,后面 3976 字节会**永远读不到**(ET 不再通知)——`wc -c` 就会小于 5000。这正是"ET 必须循环读"的实证。命令里的 `| wc -c` 就是数回显字节数,直接看它是不是 `5000` 即可。
...
```text
(gdb) break main.cpp:66      ← 断在 epoll_wait 那行(以你代码的实际行号为准)
(gdb) run
